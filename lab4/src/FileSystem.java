...
public class FileSystem {
    ...
    public FileSystem(String filepath, int memory) {
        ...
        int firstDataBlock =
                (1 + blocksBitmapsCount + inodesBitmapsCount + inodesTable);
    ...
    public String getListOfFiles(int curDir) {
        ...
        for (int i = 0; i < length && inode.numberOfBlocks[i] != -1; i++) {
            ...
            Link[] links = new Link[blockSize / linkSize];
     ...
     * @return result of creating directory
     */
    public boolean createDirectory(int curDir, String name) {
        if (superBlock.FREE_BLOCKS_COUNT == 0) {
            return false;
        }
        ...
        createLink(curDir, freeInode, name);
        ...
        links[0].set(freeInode, "."); // Cur dir
        links[1].set(curDir, ".."); // Parent dir
        Block b = new Block(links);
        b.writeSector(b.getBytes(), 0);
        writeDataBlock(b, freeBlock);

        return true;
    }

    /**
     * Remove directory in current directory if directory haven't file without
     * directories "." and ".."
     *
     * @param curDir current directory
     * @param name   name of directory
     ...
     */
    public boolean removeDirectory(int curDir, String name) {
        int blockSize = superBlock.BLOCK_SIZE;
        int linkSize = superBlock.LINK_SIZE;

        int directory = searchInode(curDir, name, false);

        Inode dir = loadInode(directory);

        for (int i = 2; i < dir.numberOfBlocks.length; i++) {
            if (dir.numberOfBlocks[i] == -1) {
                break;
            }

            Link[] links = new Link[blockSize / linkSize];
            Block b = loadDataBlock(dir.numberOfBlocks[i]);

            // Look for any files
            int length = blockSize / linkSize;
            for (int j = 0; j < length; j++) {
                links[j] = new Link();
                links[j].load(b.getSector(j * linkSize, (j + 1) * linkSize));
                if (links[j].getInodeId() != -1) {
                    return false;
                }
            }
        ...
        setToBlocksBitMap(dir.numberOfBlocks[0], 0);
        setToBlocksBitMap(dir.numberOfBlocks[1], 0);
        setToInodesBitMap(directory, 0);

        deleteLink(curDir, name);
     ...
     * @return descriptor
     */
    public int searchDirectory(int curDir, String name) {
        int blockSize = superBlock.BLOCK_SIZE;
        int linkSize = superBlock.LINK_SIZE;
        ...
        for (int i: dir.numberOfBlocks) {
            if (i == -1) {
                return -1;
            }

            Block b = loadDataBlock(i);

            for (int j = 0; j < links.length; j++) {
                links[j] = new Link();
                byte[] bytes = b.getSector(j * linkSize, (j + 1) * linkSize);
                links[j].load(bytes);
                if (links[j].getFilename().equals(name)) {
                    Inode inode = loadInode(links[j].getInodeId());

                    if (inode.fileType != Inode.DIRECTORY) {
                        return -1;
                    }

                    return links[j].getInodeId();
                }
            }
        }
     ...
     * @param filename1 filename exist file
     * @param filename2 filename link to exist file
     * @return result of creating symlink
     */
    public boolean createSymlink(int curDir, String filename1,
                                 String filename2) {
        if (superBlock.FREE_BLOCKS_COUNT == 0) {
            return false;
        }
        ...
        Inode inode = new Inode();
        inode.fileType = Inode.SYMLINK;
        inode.size = 0;
        inode.links = 1;
        inode.numberOfBlocks[0] = freeBlock;
        writeInode(freeInode, inode);

        createLink(curDir, freeInode, filename2);

        superBlock.FREE_BLOCKS_COUNT--;
        superBlock.FREE_INODES_COUNT--;
        freeMemory--;

        char[] name = (filename1 + "\0").toCharArray();
        ByteBuffer bb = ByteBuffer.allocate(superBlock.BLOCK_SIZE);
     ...
     * @return  path in symlink
     */
    public String targetInode(int i) {
        Inode inode = loadInode(i);

        Block b = loadDataBlock(inode.numberOfBlocks[0]);

        ByteBuffer bb = ByteBuffer.wrap(b.getBytes());
